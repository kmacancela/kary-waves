...
const Header = () => {
  ...
  // Render header via portal to avoid #root transform breaking fixed positioning
  const headerElement = (
    <header
    ...
    >
      <div className="container mx-auto px-6 md:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={toggle}
            className={`p-2 transition-opacity hover:opacity-70 ${getTextColor()} ${isMobileMenuOpen ? 'max-lg:hidden' : ''}`}
      ...
      <nav className={`absolute right-0 top-0 bottom-0 w-[85%] sm:w-[70%] px-6 sm:px-8 pt-28 flex flex-col gap-5 sm:gap-6 transition-transform duration-300 ease-out ${
        ...
        {navLinks.map((link) => (
          <a
            ...
            href={link.href}
            onClick={() => {
              // Remove class immediately to reset transform before navigation
              document.body.classList.remove('mobile-menu-open')
  ...
  return (
    <>
      ...
      {createPortal(mobileMenuElement, document.body)}
    </>
  )
}
